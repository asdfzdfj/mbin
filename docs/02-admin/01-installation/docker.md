...
If you want to build your own image, run (_no_ need to update the `compose.yml` file):
...
1. Choose your Redis password, PostgreSQL password, RabbitMQ password, and Mercure password.  
   Use the following command to quickly generate all the needed secrets:

```sh
for secret in APP_SECRET RABBITMQ_PASSWORD; do echo "$secret=$(openssl rand -hex 16)"; done
for secret in POSTGRES_PASSWORD REDIS_PASSWORD MERCURE_JWT_SECRET; do echo "$secret=$(openssl rand -hex 32)"; done
```
...
4. Update `APP_SECRET` in `.env` with value generated above.
...
> [!IMPORTANT]
> Ensure the `HTTPS` environmental variable is set to `TRUE` in `compose.override.yml` for the `php` and `messenger` containers **if your environment is using a valid certificate behind a reverse proxy**.
> This is likely true for most production environments and is required for proper federation, that is, this will ensure the webfinger responses include `https:` in the URLs generated.
...
Make sure `KBIN_STORAGE_URL` in your `.env` configuration file is set to be `https://yourdomain.tld/media` (assuming you setup Nginx with SSL certificate at that endpoint by now).

You can also serve those media files on another server by mirroring the files at `storage/media` and changing `KBIN_STORAGE_URL` accordingly.
...
Run the containers and make sure that everything starts:
...
See your running containers via: `docker compose ps`.
...
If everything is all right, stop the containers (`Ctrl-C`) and then re-run the containers in detached mode:
...
> [!IMPORTANT]
> The docker instance can be reached at [http://127.0.0.1:8008](http://127.0.0.1:8008),
> we strongly advise you to put a reverse proxy (like Nginx) in front of the docker instance.
> Nginx could listen on ports 80 and 443 and Nginx should handle SSL/TLS offloading.
> See the following section.

### Mbin NGINX Server Block

See [NGINX Configuration Guide](../02-configuration/nginx.md#general-nginx-configs) for general configuration.
